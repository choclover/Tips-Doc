...
public class MD5 {
    ...
    public static final String toHex( final byte hash ) {
		char buf[] = new char[2];
        buf[0] = HEX_CHARS[(hash >>> 4) & 0xf];
        buf[1] = HEX_CHARS[hash & 0xf];
		return new String(buf);
	}
	...
	public static byte[] Base64Decode(final byte[] data) 
	{
		...
		{
			chunk[0] = GetBase64Value(data[i]); 
			chunk[1] = GetBase64Value(data[i+1]); 
			chunk[2] = GetBase64Value(data[i+2]); 
			chunk[3] = GetBase64Value(data[i+3]); 

			result[counter++] = (byte)((chunk[0] << 2) | (chunk[1] >> 4)); 
			result[counter++] = (byte)((chunk[1] << 4) | (chunk[2] >> 2)); 
			result[counter++] = (byte)((chunk[2] << 6) | (chunk[3]));
		}
		return result;
	} 
